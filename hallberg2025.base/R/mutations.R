...
#' version, hence pinning it in `uvr.toml`/`uvr.lock`).
#'
...
fit_mutational_signatures <- function(sig_input) {
  ...
  ## `signatures.cosmic` and `tri.counts.exome` are LazyData: they live in
  ## deconstructSigs' lazy-load database, which is only on the search path
  ## once the package is ATTACHED. The archived producer script got them for
  ## free from its `library(deconstructSigs)` call; a package function must
  ## not attach anything, so both are loaded explicitly with utils::data()
  ## and passed in.
  ds <- new.env(parent = emptyenv())
  utils::data("signatures.cosmic", "tri.counts.exome",
              package = "deconstructSigs", envir = ds)

  ## Passing the ratio data frame is exactly what tri.counts.method = "exome"
  ## does internally -- deconstructSigs:::getTriContextFraction()'s "exome"
  ## branch computes `1 / tri.counts.exome` and hands it to norm.it(), which
  ## is the same value its data-frame branch uses verbatim. Naming the string
  ## instead would make the internals look up `tri.counts.exome` in their own
  ## namespace, where unattached LazyData is not visible.
  exome_ratio <- 1 / ds$tri.counts.exome

  output <- lapply(rownames(sigs_input), function(id) {
    deconstructSigs::whichSignatures(
      tumor.ref         = sigs_input,
      signatures.ref    = ds$signatures.cosmic,
      sample.id         = id,
      contexts.needed   = TRUE,
      tri.counts.method = exome_ratio
    )
  })
...
#'
#' Files are individual per-sample Strelka rerun reports, rsynced from the
#' filtered small-variant output of the lab's JHPCE Strelka pipeline into
#' \code{extdata/strelka_reruns/} (one
