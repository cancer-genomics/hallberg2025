...
##      Its producer is the hallberg2025.seq.data/data-raw Track C reproduction; see
##      card SEQ-16.
...
read_trellis_directories <- function(listing_file, mirror_dir) {
  ...
  stop(
    ...
    "\n",
    "  What it is: the per-sample output directories of the trellis run, held on\n",
    "    the lab's JHPCE storage -- the trellis .temp tree plus the re-genotyped\n",
    "    rerun tree alongside it. Neither is reachable from a public clone.\n",
    ...
    "    hallberg2025.seq.data/data-raw Track C reproduction -- see card SEQ-16.\n",
...
list(
    ...
    ##
    ## Requires `deconstructSigs` (pinned in uvr.toml/uvr.lock to the CRAN
    ## GitHub mirror at the 1.8.0 tag; the package is archived on CRAN) and
    ## `BSgenome.Hsapiens.UCSC.hg18`. Both install and load correctly, but
    ## ONLY under the R that owns .uvr/library -- run them via
    ## `scripts/bioc_install_env.sh`, not the framework Rscript (ENV-13).
    ##
    ## These targets do NOT reproduce the committed .rds files, and are not
    ## expected to. BASE-02 verified the port against the 2019 producer's own
    ## saved per-sample output: 37 of 53 shared endometrioid samples reproduce
    ## its trinucleotide profile bit-exactly, confirming the parsing, arm
    ## membership and deconstructSigs wiring here are correct. The residual
    ## divergence is in the INPUT: the committed matrices were fitted on a
    ## 2019 mutation table that no longer exists, not on extdata/mutations.tsv.
    ## See cards/BASE-02-port-mutational-signatures.md's Notes/log.
